...
from src.root_path import DATA_ROOT_PATH
...
class HotPotQAWrapper(gym.Wrapper):
    def __init__(self, env, split):
        super().__init__(env)
        data_file = os.path.join(DATA_ROOT_PATH, "HotpotQA", "hotpot_dev_distractor_v1.json")
...
class StrategyQAWrapper(HotPotQAWrapper):
    def __init__(self, env, split):
        super(HotPotQAWrapper, self).__init__(env)
        data_file = os.path.join(DATA_ROOT_PATH, "StrategyQA", "strategyqa_train.json")
        self.data = json.load(open(data_file))
        self.data = [(d['question'], "Yes" if d["answer"] == True else "No") for d in self.data]
        self.data_idx = 0
        self.split = split
...
class FeverWrapper(gym.Wrapper):
    def __init__(self, env, split):
        ...
        data_path = os.path.join(DATA_ROOT_PATH, "FEVER2.0", "fever2-fixers-dev.jsonl")
