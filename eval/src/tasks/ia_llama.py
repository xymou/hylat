...
def call_llm(prompt: str, client, model_name: str, max_tokens: int = 1024, temperature: float = 1.0) -> str:
    """
    Wrapper for an OpenAI-like chat completion call.
    
    Args:
        - prompt: the input prompt
        - client: OpenAI API client
        - model_name: string model identifier
        - max_tokens: max_tokens for generation
        - temperature: temperature for generation
    Returns:
        - the model output text (str).
    """
...
class IATask(BaseTask):
    def __init__(self, agents, dataset, args):
        ...
        self.planner_name = os.getenv("PLANNER_NAME")
        self.planner = OpenAI(api_key=api_key, base_url=base_url)
        
    def generate_sub_question(self, segments, question):
        prompt = """Given a question and the following document(s), generate a relevant sub-question that can be answered:\n\nQuestion: {question}\nDocuments: {segments}\n\nReturn a JSON object only:{{"sub_question": "..."}}"""
        prompt = prompt.format(question=question, segments=segments)
        max_try = 3
        for _ in range(max_try):
            try:
                res = call_llm(prompt, self.planner, self.planner_name, 256, 0)
                sub_question = json.loads(res)["sub_question"]
                return sub_question
            except:
                continue
    ...
    def run(self, data):
        ...
        for agent, allo_psg in zip(self.agents, allocate_psg):
            ...
            sub_question = self.generate_sub_question(segments, question)
            agent.sub_question = sub_question
...
class SingleIATask(IATask):
    def run_func(self, prompts):
        ...
        for agent in self.agents:
            prompt = prompts["direct_prompt"].replace("{segments}", agent.private_knowledge).replace("{sub_question}", agent.sub_question)
...
class NlIATask(IATask):
    def run_func(self, prompts):
        ...
        # 首次独立出去
        for agent in self.agents:
            agent.init_history(first_user_prompt=prompts["first_prompt"].replace("{segments}", agent.private_knowledge).replace("{sub_question}", agent.sub_question))
        ...
        for cur in range(agent_cnt):
            agent = self.agents[cur]
            final_prompt = "Stop asking and answering questions. What's your final answer to the original problem?\nPlease respond in the form \\boxed\{answer\}."
            agent.history_msgs.append({
                "role": "user",
...
class CipherIATask(IATask):
    def run_func(self, prompts):
        ...
        for cur in range(agent_cnt):
            agent = self.agents[cur]
            final_prompt = "Stop asking and answering questions. What's your final answer to the original problem?\nPlease respond in the form \\boxed\{answer\}."
            final_prompt_emb = agent.text_to_embedding(final_prompt)
            agent.history_embs = torch.cat([
                agent.history_embs,
                agent.user_embs_fr, 
                final_prompt_emb,
...
class SDEIATask(IATask):
    def run_func(self, prompts):
        ...
        for agent in self.agents:
            final_prompt = "Stop asking and answering questions. What's your final answer to the original problem?\nPlease respond in the form \\boxed\{answer\}."
            final_prompt_ids = agent.tokenizer.encode(final_prompt, add_special_tokens=False)   
            agent.history_ids = agent.history_ids + agent.user_prompt_fr + \
                    final_prompt_ids + agent.user_prompt_ed + agent.assistant_prompt_fr
            output_ids, _ = agent.generate(
                input_ids = agent.history_ids,
                if_edit=False,
                edit_layer_idx=self.args.edit_layer_idx,
